...
const Mapping = () => {
    ...
    client.on("message", (TOPIC_DISTANE, message) => {
        let newX = cellSide * ((Math.cos(((2*Math.PI / THREE_SIXTY) * angle))) * message); // get x value
        let newY = cellSide * ((Math.sin(((2*Math.PI / THREE_SIXTY) * angle))) * message); // get y value
        // reposition previos positions depending on movements
        const ctx = document.getElementById("canvas").getContext("2d");
        
            for(let y = 0; y < HEIGHT; y++){                
                for( let x = 0; y < WIDTH; y++){
                    if(x+ newX >= 0 && y + newY >= 0 && x+ newX < WIDTH  && y+newY < HEIGHT){
